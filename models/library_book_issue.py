...
class LibraryBookIssue(models.Model):
    ...
    fine_rate = fields.Float(
        ...
        default=2.0,
        help='Fine amount charged per overdue day.',
    )
    ...
    def _update_stock(self, qty_change):
        location = self.env.ref(
            'library_management.stock_location_library_shelf',
            raise_if_not_found=False,
        )
        if not location:
            location = self.env['stock.location'].search(
                [('name', 'ilike', 'Library Shelf'),
                 ('usage', '=', 'internal')],
                limit=1,
            )
        if not location:
        ...
        if quant:
            quant.sudo().write({'quantity': quant.quantity + qty_change})
        else:
